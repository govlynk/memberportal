...
export const useUserCompanyStore = create((set, get) => ({
  ...
  removeUserCompanyAssociation: async (userCompanyRoleId) => {
    try {
      ...
      set((state) => {
        const newCompanies = state.userCompanies.filter(
          (uc) => uc.userCompanyRoleId !== userCompanyRoleId
        );
        
        // Update active company if the removed one was active
        const removedCompany = state.userCompanies.find(
          (uc) => uc.userCompanyRoleId === userCompanyRoleId
        );
  ...
  updateUserCompanyRole: async (userCompanyRoleId, updates) => {
    try {
      const updatedAssociation = await client.models.UserCompanyRole.update({
        id: userCompanyRoleId,
        ...updates,
      });

      set((state) => ({
        userCompanies: state.userCompanies.map((uc) =>
          uc.userCompanyRoleId === userCompanyRoleId
            ? { ...uc, ...updates }
            : uc
        ),
        error: null,
      ...
      console.error("Update user company role error:", err);
